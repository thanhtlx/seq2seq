...
#     def __init__(self, w2v, tokenizer, hidden_dim=128, lstm_layer=2, dropout=0.2):
...
#         #     hidden_dim*2, batch_first=True)  # 2 is bidrectional
#         self.fc = nn.Sequential(nn.Linear(hidden_dim*2, hidden_dim),
...
#         # x, _ = self.atten(out, attention_mask)  # skip connect
#         # print(x.shape)
#         x = self.fc(self.dropout(out))
#         return x

#     def embeddings(self, ids):
#         # batchxlenxhiden
#         result = np.zeros((ids.shape[0], ids.shape[1], self.hidden_size))
#         for i in range(ids.shape[0]):
#             # tokens = self.tokenizer.convert_ids_to_tokens(ids[i])
#             tokens = [self.w2v.index_to_key[idx] for idx in ids[i]]
#             # print(tokens)
#             for j in range(len(tokens)):
#                 if tokens[j] in self.w2v:
#                     result[i][j] = self.w2v[tokens[j]]
#             # if i <= 0:
#             #     print(result[i])
#         with torch.no_grad():
#             result = torch.tensor(
#                 result, dtype=torch.float, device=self.device)
#             return result
...
class Encoder(nn.Module):
    def __init__(self, config, lstm_layer=2, dropout=0.2):
        ...
        self.lstm = nn.LSTM(input_size=self.hidden_size,
                            ...
                            num_layers=lstm_layer,
                            bidirectional=True)
        self.fc = nn.Sequential(nn.Linear(self.hidden_size*2, self.hidden_size),
                                nn.ReLU())
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() else "cpu")
