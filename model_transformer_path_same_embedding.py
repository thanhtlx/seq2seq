...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, beam_size=None, max_length=None, sos_id=None, eos_id=None,tokenizer):
        super(Seq2Seq, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.config = config
        self.tokenizer = tokenizer
        self.w2v = KeyedVectors.load("w2v600", mmap='r')
    ...
    def embeddings(self,ids):
        embeddings = torch.zeros((ids.shape[0],ids.shape[1],256)) 
        for idx, id in enumerate(ids):
            tokens = self.tokenizer.conver_id_to_token(id)
            for jdx,token in enumerate(tokens):
                if token in self.w2v.key_to_index.keys():
                    embeddings[idx][jdx] = torch.tensor(self.w2v[token])
